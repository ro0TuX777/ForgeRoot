...
def collect_python_units(project_root: Path, project_id: str, src_root: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    modules: Dict[str, Dict[str, Any]] = {}
    # Support configurable source root — defaults to project_root/src but falls back
    # to project_root itself when no src/ exists (e.g. SAM's flat layout).
    if src_root is None:
        candidate = project_root / "src"
        src_root = candidate if candidate.exists() else project_root
...
def assemble_units(project_root: Path, project_id: str, src_root: Optional[Path] = None) -> List[Dict[str, Any]]:
    units: Dict[str, Dict[str, Any]] = {}
    # Python units use the configurable src_root
    units.update(collect_python_units(project_root, project_id, src_root=src_root))
    for collector in (
...
def run(project_context: Dict[str, Any], link_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Allow callers to specify a custom source root (e.g. for flat repo layouts like SAM)
    config = link_config.get("config", {}) if isinstance(link_config, dict) else {}
    src_root_override = config.get("src_root")
    src_root = Path(src_root_override) if src_root_override else None

    units = assemble_units(project_root, project_id, src_root=src_root)
